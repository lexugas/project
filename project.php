		  ...
		  <div class="modal fade" id="myModal">
			...
			<!-- The Modal -->
			  ...
			  <div class="container jumbotron">
			  	<center><span class=" " style="font-size: 3vw;text-transform: uppercase;">projects</span><hr></center>
...
if (isset($_GET['softeng'])){
    $sql = "SELECT * FROM docs WHERE docs_status = 1 AND docs_type1 = 'Software Engineering' ORDER BY docs_title ASC LIMIT " . $this_page_first_result . "," .  $results_per_page;;
}elseif (isset($_GET['microp'])) {
	$sql = "SELECT * FROM docs WHERE docs_status = 1 AND docs_type1 = 'Micro Processor' ORDER BY docs_title ASC LIMIT " . $this_page_first_result . "," .  $results_per_page;;
}elseif (isset($_GET['pd'])) {
	$sql = "SELECT * FROM docs WHERE docs_status = 1 AND docs_type1 = 'Project Design' ORDER BY docs_title ASC LIMIT " . $this_page_first_result . "," .  $results_per_page;;
}else{
	$sql = "SELECT * FROM docs WHERE docs_status = 1 ORDER BY docs_title ASC LIMIT " . $this_page_first_result . "," .  $results_per_page;;
}
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
    	...
    	echo '<a href="view.php?view='. $row["docs_id"].'"class="btn btn-outline-info btn-block p-3" style="font-size: 1.5vw;text-transform: uppercase;" role="button">'.$row['docs_title'].'</a><br>';
...
for ($page=1;$page<=$number_of_pages;$page++) {
  echo '<a href="project.php?&page=' . $page . '">' . $page . '</a> ';
  }
